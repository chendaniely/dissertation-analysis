...
for (q in unique(counts$qbase)) {
  ...
  if (q %in% c("Q2.3", "Q3.4", "Q6.2")) { # plots used for dissertation
    edt_pth <- here("..",
              "dissertation-edt",
              "figs",
              "020-persona",
...
g <- likert_q_count %>%
  ...
  ggplot(aes(x = response, y = n, fill = as.factor(group3))) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip() +
  ggtitle(stringr::str_wrap("Using a programming language (like R or Python) can make my analyses easier to reproduce.", 80)) +
  theme_minimal() +
  theme(
