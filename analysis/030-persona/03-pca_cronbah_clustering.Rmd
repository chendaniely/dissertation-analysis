...
source(here::here("./analysis/01-qualtrics/question_str_to_int.R"))
...
wide <- persona %>%
  ...
  dplyr::filter(stringr::str_starts(question_part, "Q[3|4|5|6|7]")) %>%
  dplyr::filter(!stringr::str_starts(question_part, "Q3.2")) %>% # drop "what programming language you have used in the past question
  # # fixes curly approstrphe
  dplyr::filter(!stringr::str_starts(question_part, "Q7\\.[1|3|4]")) %>% # only keep likert questions in Q7
...
# save out data before col to rownames
readr::write_csv(wide, here::here("./data/final/persona/01-participant_numeric.csv"))
...
Q5.1 and Q3.7 were causing negative correlations.
Makes sense for Q5.1, since the scale is not linear.
...
```{r}
table(wide$Q5.1) %>% addmargins()
```

Not exactly sure why Q3.7 would be showing negative correlation.
Probably because our users almost never work with databases.
...
```{r}
table(wide$Q3.7) %>% addmargins()
```


```{r}
mat <- wide %>%
  dplyr::select(-Q5.1, -Q3.7) %>%
  data.matrix()

```

test for unidimensionality
this is done by looking at the eigenvalue on the covariation matrix

```{r}
psych::alpha(mat)
```

In general. None of the items can be dropped to improve the alpha when all items are included.

These results show that the survey has high internal consistency.
