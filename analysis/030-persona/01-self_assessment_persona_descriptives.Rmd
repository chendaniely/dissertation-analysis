...
stopifnot(all(table(num_consented$id_person) == 2)) # this is also a duplicate ID check
...
stopifnot(any(duplicated(num_consented$id_person)) == FALSE)

table(num_consented$qualify)

num_consented <- length(num_consented$id_person)
```
...
```{r}
researcher_group <- c("Academic", "faculty", "PhD", "PhD (Professor)", "Analyst", "Research administrator", "Research Adminstrator", "healthcare administration, PhD, MPH", "Biomedical Technician", "iTHRIV Scholar", "medical librarian", "Postdoc", "Librarian")

occupation <- counts %>%
  dplyr::filter(qbase == "Q2.3") %>%
  dplyr::mutate(
    group = dplyr::case_when(
      response %in% researcher_group ~ "researcher",
      response %in% c("RN/PA", "DO/MD", "DVM", "PharmD") ~ "clinician",
      response %in% c("Student (DVM)") ~ "clinical_student",
      response %in% c("Student (Graduate)", "Student (Masters e.g., MPH)", "Student (Undergraduate)") ~ "student",
      TRUE ~ NA_character_
...
table(occupation$response, occupation$group, useNA = "always") %>% addmargins()
```
