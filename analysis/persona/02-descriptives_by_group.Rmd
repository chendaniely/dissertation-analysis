...
```{r}
likert_q_count5_prop <- likert_questions5 %>%
  dplyr::group_by(group5, text) %>%
  dplyr::count(response) %>%
  dplyr::filter(!is.na(group5)) %>%
  dplyr::group_by(group5) %>%
...
```{r}
ggsave(filename = here("./output", "persona", "group_descriptives_5", "likert_prop.png"),
       plot = g,
       width = 13,
       height = 8,
       units = "in")
...
reponse_table %>%
  ...
  kableExtra::row_spec(which(reponse_table$group5 == 1), color = "white", background = .GlobalEnv$COLORS[1]) %>%
  kableExtra::row_spec(which(reponse_table$group5 == 2), color = "white", background = .GlobalEnv$COLORS[2]) %>%
  kableExtra::row_spec(which(reponse_table$group5 == 3), color = "white", background = .GlobalEnv$COLORS[3]) %>%
  kableExtra::row_spec(which(reponse_table$group5 == 4), color = "white", background = .GlobalEnv$COLORS[4]) %>%
  kableExtra::row_spec(which(reponse_table$group5 == 5), color = "white", background = .GlobalEnv$COLORS[5])
